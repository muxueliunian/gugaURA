...
impl Interceptor {
    ...
    pub fn hook(&self, orig_addr: usize, hook_addr: usize) -> Result<usize, String> {
        unsafe {
            let trampoline =
                MinHook::create_hook(orig_addr as *mut c_void, hook_addr as *mut c_void)
                    .map_err(|e| format!("MinHook create error: {:?}", e))?
                    as usize;
...
extern "C" fn hooked_load_library_w(filename: PCWSTR) -> HMODULE {
    ...
    let orig_fn: LoadLibraryWFn = unsafe {
        let trampoline = GugaURA::instance()
            .interceptor
            .get_trampoline(
                windows::Win32::System::LibraryLoader::GetProcAddress(
                    windows::Win32::System::LibraryLoader::GetModuleHandleW(w!("kernel32.dll"))
    ...
    // 检查是否是我们关心的DLL
    let filename_str = unsafe { filename.to_string().unwrap_or_default() };
        ...
        || filename_str.to_lowercase().contains("cri_mana_vpx")
    {
        ...
        // 尝试获取GameAssembly
        let ga_handle = unsafe { WinLoadLibraryW(w!("GameAssembly.dll")) };
